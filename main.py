...
test_object = Polygon([Vector2(0 ,800),Vector2(1300, 800),Vector2(1300, 900),Vector2(0, 900)],40000000000)
...
second_object = circle(Vector2(100,100),3,50)
...
while running:
    ...
    if keys[pygame.K_RIGHT] and not key_state_1[pygame.K_RIGHT]:
        test_object.add(Vector2(newton_to_force(30), 0))  # Apply force to the right
        key_state_1[pygame.K_RIGHT] = True

    if keys[pygame.K_LEFT] and not key_state_1[pygame.K_LEFT]:
        test_object.add(Vector2(-newton_to_force(30), 0))  # Apply force to the left
        key_state_1[pygame.K_LEFT] = True

    if keys[pygame.K_DOWN] and not key_state_1[pygame.K_DOWN]:
        test_object.add(Vector2(0, newton_to_force(30)))  # Apply force downward
        key_state_1[pygame.K_DOWN] = True

    if keys[pygame.K_UP] and not key_state_1[pygame.K_UP]:
        test_object.add(Vector2(0, -newton_to_force(30)))  # Apply force upward
    ...
    if keys_2[pygame.K_d] and not key_state_2[pygame.K_d]:
        second_object.add(Vector2(newton_to_force(46), 0))
    ...
    if keys_2[pygame.K_s] and not key_state_2[pygame.K_s]:
        second_object.add(Vector2(0, newton_to_force(46)))
    ...
    if keys_2[pygame.K_e] and keys_2[pygame.K_r]:
        display_width, display_height = 1000, 800
    ...
    second_object.velocity += Vector2(0,2)
    gjk = GJK2D(test_object,second_object)
    trig = gjk.detection()
    stuff = gjk.EPA(trig)
    if trig is not None:
    ...
    test_object.draw(screen,(255,0,0))
    second_object.draw(screen,(0,255,0))
    
    # Display debug positions
    font = pygame.font.SysFont("Arial", 24)
    position_text_1 = font.render(f"Position 1: ({int(test_object.centroid.x)}, {-int(test_object.centroid.y)})", True, (255, 255, 255))
    position_text_2 = font.render(f"Position 2: ({int(second_object.centroid.x)}, {-int(second_object.centroid.y)})", True, (255, 255, 255))
